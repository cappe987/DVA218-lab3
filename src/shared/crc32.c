...
unsigned int crc32(const unsigned char* buf, size_t len) {
  return calculate_crc32(0xffffffff, buf, len) ^ 0xffffffff;
}


// Make sure there is no garbage values in the char array.
// Although if we make it the exact size, only the base_packet data could 
// contain garbage values. Which is a problem for someone else.
// But does it really matter though? 
crc_packet create_crc(char data[CRC_DATA_SIZE]){ // Replace with struct 
  crc_packet packet;
  memcpy(packet.data, data, CRC_DATA_SIZE);
